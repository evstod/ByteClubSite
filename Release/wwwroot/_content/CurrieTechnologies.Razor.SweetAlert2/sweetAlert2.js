...
function dispatchQueuePreDeny(requestId, inputValue) {
  const valArray = Array.isArray(inputValue) ? inputValue : [inputValue];
  return DotNet.invokeMethodAsync(namespace, "ReceivePreDenyQueueInput", requestId, valArray.map(v => getStringVersion(v)));
}
...
function getSwalSettingsFromPoco(settings, requestId, isQueue) {
  ...
  if (settings.preConfirm) {
    swalSettings.preConfirm = isQueue ? inputValue => dispatchQueuePreConfirm(requestId, inputValue).then(processPreConfirmDenyResult) : inputValue => dispatchPreConfirm(requestId, inputValue).then(processPreConfirmDenyResult);
  } else {
    delete swalSettings.preConfirm;
  }

  if (settings.preDeny) {
    swalSettings.preDeny = isQueue ? inputValue => dispatchQueuePreDeny(requestId, inputValue).then(processPreConfirmDenyResult) : inputValue => dispatchPreDeny(requestId, inputValue).then(processPreConfirmDenyResult);
  } else {
